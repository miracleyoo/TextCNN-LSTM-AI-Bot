...
class LSTM(BasicModule):
    ...
    def __init__(self, opt):
        ...
        self.lstm             = nn.LSTM(self.embedding_dim, self.hidden_dim, 
            self.number_layers, batch_first=True, dropout=0.5, bidirectional=False)
        self.fc               = nn.Linear(self.hidden_dim, self.output_dim)
        self.hidden           = self.init_hidden()
        
    def init_hidden(self):
        if self.use_cuda:
            return (Variable(torch.zeros(self.number_layers, self.batch_size, self.hidden_dim).cuda()),
                Variable(torch.zeros(self.number_layers, self.batch_size, self.hidden_dim)).cuda())
        else:
            return (Variable(torch.zeros(self.number_layers, self.batch_size, self.hidden_dim)),
                Variable(torch.zeros(self.number_layers, self.batch_size, self.hidden_dim)))
